...
function RemoveMoney(account, amount)
	...
	if amount > 0 then
		...
		if tonumber(Accounts[account]) >= amount then
			Accounts[account] = Accounts[account] - amount
			isRemoved = true
		end

		MySQL.update('UPDATE management_funds SET amount = ? WHERE job_name = ? and type = "boss"',
...
RegisterNetEvent("qb-bossmenu:server:withdrawMoney", function(amount)
	...
	if RemoveMoney(job, amount) then
		...
		TriggerEvent('qb-log:server:CreateLog', 'bossmenu', 'Withdraw Money', "blue",
			Player.PlayerData.name .. "Withdrawal $" .. amount .. ' (' .. job .. ')', false)
...
RegisterNetEvent("qb-bossmenu:server:depositMoney", function(amount)
	...
	if Player.Functions.RemoveMoney("cash", amount) then
		local job = Player.PlayerData.job.name
		AddMoney(job, amount)
		TriggerEvent('qb-log:server:CreateLog', 'bossmenu', 'Deposit Money', "blue",
			Player.PlayerData.name .. "Deposit $" .. amount .. ' (' .. job .. ')', false)
...
RegisterNetEvent('qb-bossmenu:server:GradeUpdate', function(data)
	...
	if data.grade > Player.PlayerData.job.grade.level then TriggerClientEvent('QBCore:Notify', src,
			"You cannot promote to this rank!", "error")
		return
	end
...
RegisterNetEvent('qb-bossmenu:server:FireEmployee', function(target)
	...
	if Employee then
		if target ~= Player.PlayerData.citizenid then
			if Employee.PlayerData.job.grade.level > Player.PlayerData.job.grade.level then TriggerClientEvent('QBCore:Notify',
					src, "You cannot fire this citizen!", "error")
				return
			end
		...
		if player[1] ~= nil then
			...
			if Employee.job.grade.level > Player.PlayerData.job.grade.level then TriggerClientEvent('QBCore:Notify', src,
					"You cannot fire this citizen!", "error")
				return
			end
			local job = {}
			job.name = "unemployed"
...
RegisterNetEvent('qb-bossmenu:server:HireEmployee', function(recruit)
	...
	if Target and Target.Functions.SetJob(Player.PlayerData.job.name, 0) then
		...
		TriggerClientEvent('QBCore:Notify', Target.PlayerData.source, "You were hired as " .. Player.PlayerData.job.label .. ""
			, "success")
