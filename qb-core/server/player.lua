    ...
    if source and source ~= '' then
        if citizenid then
            ...
            if PlayerData and license == PlayerData.license then
               ...
               if PlayerData.business then
                    PlayerData.business = json.decode(PlayerData.business)
                else
                    PlayerData.business = {}
                end
...
function QBCore.Player.CheckPlayerData(source, PlayerData)
    ...
    -- Business Modified QB Core For AIVP 
    if PlayerData.business and PlayerData.business.name and not QBCore.Shared.Businesss[PlayerData.business.name] then PlayerData.business = nil end
    PlayerData.business = PlayerData.business or {}
    PlayerData.business.name = PlayerData.business.name or 'nobusiness'
    PlayerData.business.label = PlayerData.business.label or 'None'
    PlayerData.business.payment = PlayerData.business.payment or 10
    PlayerData.business.type = PlayerData.business.type or 'foodshop'
   
    print(QBCore.Shared.Businesss[PlayerData.business.name].defaultDuty)
    if QBCore.Shared.ForceBusinessDefaultDutyAtLogin or PlayerData.business.onduty == nil then
        PlayerData.business.onduty = false
    end
    PlayerData.business.isboss = PlayerData.business.isboss or false
    PlayerData.business.grade = PlayerData.business.grade or {}
    PlayerData.business.grade.name = PlayerData.business.grade.name or 'Entrepreneur'
    PlayerData.business.grade.level = PlayerData.business.grade.level or 0
...
-- 
function self.Functions.SetBusiness(business, grade)
    business = business:lower()
    grade = tostring(grade) or '0'
    if not QBCore.Shared.Businesss[business] then return false end
    self.PlayerData.business.name = business
    self.PlayerData.business.label = QBCore.Shared.Businesss[business].label
    self.PlayerData.business.onduty = QBCore.Shared.Businesss[business].defaultDuty
    self.PlayerData.business.type = QBCore.Shared.Businesss[business].type or 'none'
    if QBCore.Shared.Businesss[business].grades[grade] then
        local businessgrade = QBCore.Shared.Businesss[business].grades[grade]
        self.PlayerData.business.grade = {}
        self.PlayerData.business.grade.name = businessgrade.name
        self.PlayerData.business.grade.level = tonumber(grade)
        self.PlayerData.business.payment = businessgrade.payment or 30
        self.PlayerData.business.isboss = businessgrade.isboss or false
    else
        self.PlayerData.business.grade = {}
        self.PlayerData.business.grade.name = 'No Grades'
        self.PlayerData.business.grade.level = 0
        self.PlayerData.business.payment = 30
        self.PlayerData.business.isboss = false
    end

    if not self.Offline then
        self.Functions.UpdatePlayerData()
        TriggerEvent('QBCore:Server:OnBusinessUpdate', self.PlayerData.source, self.PlayerData.job)
        TriggerClientEvent('QBCore:Client:OnBusinessUpdate', self.PlayerData.source, self.PlayerData.job)
    end
...
--
    ...
    function self.Functions.SetBusinessDuty(onDuty)
        self.PlayerData.business.onduty = not not onDuty -- Make sure the value is a boolean if nil is sent
        self.Functions.UpdatePlayerData()
    end
...
function QBCore.Player.Save(source)
    ...
    if PlayerData then
        MySQL.insert('INSERT INTO players (citizenid, cid, license, name, business, money, charinfo, job, gang, position, metadata) VALUES (:citizenid, :cid, :license, :name, :business, :money, :charinfo, :job, :gang, :position, :metadata) ON DUPLICATE KEY UPDATE cid = :cid, name = :name, business = :business, money = :money, charinfo = :charinfo, job = :job, gang = :gang, position = :position, metadata = :metadata', {
            citizenid = PlayerData.citizenid,
            cid = tonumber(PlayerData.cid),
            license = PlayerData.license,
            name = PlayerData.name,
            business = json.encode(PlayerData.business),
...
function QBCore.Player.SaveOffline(PlayerData)
    if PlayerData then
        MySQL.Async.insert('INSERT INTO players (citizenid, cid, license, name, business, money, charinfo, job, gang, position, metadata) VALUES (:citizenid, :cid, :license, :name, :business, :money, :charinfo, :job, :gang, :position, :metadata) ON DUPLICATE KEY UPDATE cid = :cid, name = :name, money = :money, charinfo = :charinfo, job = :job, gang = :gang, position = :position, metadata = :metadata', {
            citizenid = PlayerData.citizenid,
            cid = tonumber(PlayerData.cid),
            license = PlayerData.license,
            name = PlayerData.name,
