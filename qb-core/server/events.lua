...
RegisterNetEvent('QBCore:ToggleBusiness', function()
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return end
    if Player.PlayerData.business.onduty then
        Player.Functions.SetBusinessDuty(false)
        TriggerClientEvent('QBCore:Notify', src, Lang:t('info.off_duty'))
    else
        Player.Functions.SetBusinessDuty(true)
        TriggerClientEvent('QBCore:Notify', src, Lang:t('info.on_duty'))
    end
    -- TriggerClientEvent('QBCore:Client:SetDuty', src, Player.PlayerData.job.onduty)
end)
...
RegisterNetEvent('QBCore:CallCommand', function(command, args)
    ...
    if hasPerm then
        if QBCore.Commands.List[command].argsrequired and #QBCore.Commands.List[command].arguments ~= 0 and
            not args[#QBCore.Commands.List[command].arguments] then
