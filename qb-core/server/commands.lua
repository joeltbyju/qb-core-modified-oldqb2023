...
QBCore.Commands.Add('business', 'Business', {}, false, function(source)
    local PlayerBusiness = QBCore.Functions.GetPlayer(source).PlayerData.business
    TriggerClientEvent('QBCore:Notify', source, Lang:t('info.business_info', {value = PlayerBusiness.label, value2 = PlayerBusiness.grade.name, value3 = PlayerBusiness.onduty}))
end, 'user')
...
QBCore.Commands.Add('setbusiness', 'Set Business Of Player', { { name = 'ID', help = 'Player ID' }, { name = 'Business', help = 'Business Name' }, { name = 'Grade', help = 'Business Grade' } }, true, function(source, args)
    local Player = QBCore.Functions.GetPlayer(tonumber(args[1]))
    if Player then
        Player.Functions.SetBusiness(tostring(args[2]), tonumber(args[3]))
    else
        TriggerClientEvent('QBCore:Notify', source, Lang:t('error.not_online'), 'error')
    end
end, 'admin')
